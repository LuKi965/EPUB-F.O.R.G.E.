...
CATALOGUE: dict[str, str] = {
    ...
    "paragraphs.empty-runs-kept": "{count} empty paragraph(s) in {runs} run(s) — two or more in a row, at a document edge or beside a heading — stay as the file has them, as chosen; {breaks} single blank line(s) between paragraphs are breaks between scenes and were never in question",
    "paragraphs.empty-runs-unanswered": "{count} empty paragraph(s) in {runs} run(s) stay as the file has them, because nobody answered; {breaks} single blank line(s) between paragraphs were never in question",
...
CATALOGUE_PL: dict[str, str] = {
    ...
    'paragraphs.empty-runs-kept': '{count} {count:pusty akapit|puste akapity|pustych akapitów} w {runs} {runs:ciągu|ciągach|ciągach} — dwa i więcej pod rząd, na brzegu dokumentu albo przy nagłówku — zostaje tak, jak ma plik, jak wybrano; {breaks} {breaks:pojedyncza pusta linia|pojedyncze puste linie|pojedynczych pustych linii} między akapitami to przerwy między scenami i nigdy nie były w pytaniu',
    'paragraphs.empty-runs-unanswered': '{count} {count:pusty akapit|puste akapity|pustych akapitów} w {runs} {runs:ciągu|ciągach|ciągach} zostaje tak, jak ma plik, bo nikt nie odpowiedział; {breaks} {breaks:pojedyncza pusta linia|pojedyncze puste linie|pojedynczych pustych linii} między akapitami nigdy nie były w pytaniu',
