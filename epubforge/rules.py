...
CATALOGUE: dict[str, str] = {
    ...
    "typography.ellipsis-kept": "{count} run(s) of three dots were left exactly as the file has them, because that was the answer",
    ...
    "typography.conjunctions-kept": "{count} single-letter conjunction(s) can still be left at the end of a line, because that was the answer",
    ...
    "typography.quotes-kept": "{count} straight quote(s) were left exactly as the file has them, because that was the answer",
    ...
    "typography.ranges-kept": "{count} hyphen(s) between numbers were left as the file has them, because that was the answer",
    ...
    "css.invalid-value-kept": "{count} declaration(s) using the invalid value 'regular' were left alone on your answer: this stylesheet also sets italic or bold, and 'normal' would have overridden what those elements inherit",
    ...
    "css.font-stack-generic-approved": "{count} font stack(s) naming faces whose kind the book states in no number gained the generic family the font's own name or common knowledge assigns ({generics}), appended on a person's word",
    ...
    "xhtml.document-language-aligned": "the document said it was written in {was} and the publication says {now}; on your answer the document now says {now} too — it has {count} character(s), too few to be evidence of a language on their own",
    "xhtml.document-language-undecided": "the document says it is written in {document} and the publication says {publication}; it has {count} character(s), too few to be evidence either way, and nobody was there to decide — it keeps what it said",
    ...
    "nav.body-type-carried": "the publisher's epub:type=\"{value}\" on the navigation document's body was carried onto the regenerated one word for word",
    "package.time-budget-is-a-setting": "the time ceiling is a setting, not a verdict about the book: raise it with --time-budget on the command line or the time budget field in the window, and rebuild",
    ...
    "render.checked": "{count} page comparison(s) in {engine} over {documents} of {total} documents in the reading order, {screens} screen(s) of each: the rebuilt book draws the same as the source there. What was not drawn was not checked",
    "render.checked-whole": "{count} page comparison(s) in {engine} over {documents} of {total} documents in the reading order, every document down to its last screen ({screens} screen(s) the longest): the rebuilt book draws the same as the source everywhere it was drawn",
    ...
    "hyphens.class-left-alone": "{count} word(s) in the '{confidence}' class were left exactly as the file has them: this book never writes them without a hyphen, so there is no evidence either way, and nobody answered for the class",
    "hyphens.class-kept": "{count} word(s) in the '{confidence}' class were left exactly as the file has them: this book never writes them without a hyphen, so there is no evidence either way, and that was answered for the whole class at once",
    ...
    "package.attributes-fell": "the output carries fewer of some attributes assistive software depends on than the source did ({detail}); nothing in this rebuild claims to have removed them, so look at where they went",
    "package.text-uncounted": "the characters of {count} document(s) could not be counted on one side of the balance or the other ({documents}), so those documents are left out of both sides and the character balance reads {before} → {after} without them. Nothing says they lost text — it says this number does not cover them",
    "paragraphs.empty-runs-kept": "{count} empty paragraph(s) in {runs} run(s) — two or more in a row, at a document edge or beside a heading — stay as the file has them, as chosen; removing would have taken {removable} of them, since a run of up to {height} between paragraphs keeps its height, and the {breaks} single blank line(s) between paragraphs are breaks between scenes that were never in question",
    "paragraphs.empty-runs-unanswered": "{count} empty paragraph(s) in {runs} run(s) stay as the file has them, because nobody answered; removing would have taken {removable} of them, a run of up to {height} between paragraphs keeps its height, and the {breaks} single blank line(s) between paragraphs were never in question",
    "paragraphs.empty-runs-removed": "{count} empty paragraph(s) removed from {documents} document(s), on a person's word or a batch's standing answer: {shortened} run(s) between paragraphs were shortened (the tallest from {longest} to {left}), {dropped} run(s) at a document edge or beside a heading went whole, {untouched} were left at their height; the text is the same to the character, {breaks} single break(s) between scenes untouched, and the change ledger carries the entry",
    "paragraphs.empty-runs-left": "{count} empty paragraph(s) in {document} were not removed after all: what was left was not the same text, or not exactly the planned paragraphs went, so the document went back to what it was",
    ...
    "render.pdf-compared": "{count} page(s) of the fixed book were compared with the same pages of the PDF — the source drawn by PDFium, the book by {engine}, ink counted per block; at most {worst} of a page's ink was missing, against a limit of {limit} measured on faithful and damaged pages",
    "render.pdf-page-differs": "this page lost more of the source page's ink than the measured limit allows — {detail}",
    "package.pdf-characters-lost": "characters the PDF draws are not in the rebuilt book: {detail}. Counted by a second reader that knows nothing about lines or reading order, so a construct the conversion does not handle cannot hide from it. Nothing was written",
    "package.pdf-characters-changed-on-request": "the second reader counts characters the PDF draws that the rebuilt book does not carry ({detail}), and every one of them is covered by a pass that was asked for and answered: {rules}. Said rather than refused, and said rather than passed over — the count is the count",
    "package.pdf-characters-lost-beyond-consent": "characters the PDF draws are not in the rebuilt book, and the passes that were asked for do not account for all of them ({rules}): {detail}. A consented pass pays for the text it says it removed, character for character; what is left over is a loss nobody asked for. Nothing was written",
    ...
    "hyphens.kept": "{count} word(s) with a hyphen inside them were left exactly as the file has them, because that was the answer",
    ...
    "substitutions.pattern-found": "this book writes '{wrong}' where '{right}' belongs, in {count} word(s); {evidence} of them the book itself also spells correctly elsewhere, which is what makes this a pattern rather than a list of typos. Nothing was changed without being asked",
    "substitutions.replaced": "{count} word(s) across {documents} document(s) got their letter back — '{wrong}' written where '{right}' belongs — answered for as one pattern rather than word by word",
    "substitutions.left-alone": "'{wrong}' stands where '{right}' belongs in {count} word(s), and all of them were left exactly as the file has them, because nobody was asked or nobody answered",
    "substitutions.kept": "'{wrong}' stands where '{right}' belongs in {count} word(s), and all of them were left exactly as the file has them, because that was the answer",
    "substitutions.reverted": "{count} document(s) came out of the substitution pass with more changed than the words agreed to, so they went back exactly as they came in",
    ...
    "package.unresolved-references": "{count} link(s) name an anchor their target document does not have ({examples}) — the anchor may exist elsewhere in the book, as a footnote export often leaves it — and strict mode does not publish a book whose references it cannot honestly resolve; nothing was written",
    ...
    "xhtml.dead-link-removed": "{count} <link> element(s) in the document head naming files not in the book were removed; nothing they pointed at ever loaded",
    ...
    "pictures.undescribed-found": "{count} image place(s) in {files} file(s) have no usable text alternative; each file was put to a person with its evidence",
    "pictures.decorative-marked": "{count} image place(s) were marked decorative (alt=\"\" with role=\"presentation\") on a person's word",
    "pictures.described": "{count} image place(s) received the description a person supplied",
    "pictures.left-alone": "{count} image place(s) in {files} file(s) stay without a text alternative; nobody described them (e.g. {examples})",
    "tables.layout-found": "{count} table(s) in {docs} document(s) hold prose in one cell or one column and are announced to assistive software as tables; the list was put to a person",
    "tables.layout-marked": "{count} table(s) holding prose were marked as layout (role=\"presentation\") on a person's word; the text is unchanged",
    "tables.layout-left-alone": "{count} table(s) holding prose in {docs} document(s) stay announced as tables; nobody marked them",
    "kepub.marked": "{docs} document(s) received the markers a Kobo looks for: {spans} sentence span(s) and {images} image span(s) inside div#book-inner, and the style block Kobo's own files carry; the text is unchanged",
    "kepub.already-marked": "{count} document(s) already carried Kobo's sentence spans and kept them as they were",
    "kepub.name": "the output is named {name}; a Kobo chooses its own renderer by the name and uses it only for a file ending in {extension}, so this one it will open as a plain EPUB",
    "kepub.first-page-not-a-cover": "the first page in the reading order, {name}, is text rather than a cover; a Kobo lays the first page of its own file out as a full-screen cover, without margins, and the reference converter puts a blank page in front of such a chapter — this program adds no page the publisher did not write",
    ...
    "css.faces-without-src-found": "{count} @font-face rule(s) declare no src, so they can load no font and every reading system ignores them",
    "css.faces-without-src-removed": "{count} @font-face rule(s) with no src were removed: a face that names no file cannot load one",
    "css.faces-without-src-unverified": "{count} @font-face rule(s) with no src were left in place: removing them did not come out as a stylesheet",
    ...
    "css.malformed-declaration-converter-kept": "{count} declaration(s) written the way an HTML attribute is written bear a converter's signature and would have gone with the rest of its junk; the style sweep is switched off, so they stay — no reading system has ever applied them",
    "css.publisher-typo-fixed": "{count} declaration(s) broken by a publisher's typo — {names} — were repaired at a person's word, and formatting no reader had ever applied now applies",
    "css.publisher-typo-left": "{count} declaration(s) carry a publisher's typo with an obvious repair to propose; nobody answered, so they were left as they were — reading systems keep ignoring them",
    "css.publisher-typo-kept": "{count} declaration(s) carry a publisher's typo with an obvious repair to propose; the answer was to leave them, so they were — reading systems keep ignoring them",
    ...
    # -- fonts --------------------------------------------------------------
    "font.subset": "{count} embedded font(s) were cut down to the characters this book actually uses — {before} became {after}, saving {saved}",
    "font.subset-tables-dropped": "cutting the fonts down dropped tables the font library cannot carry through a subset — {names}. Most are a font editor's own bookkeeping, or belong to typography engines almost no reading system uses. One is worth knowing about: the old kerning table, `kern` — where a font carries no modern kerning beside it, letter spacing can change. The appearance check compares the pages before and after either way, and refuses the book if it lost anything",
    "font.subset-skipped-obfuscated": "this book's fonts arrived obfuscated, so they were left byte for byte as they were — subsetting them is untested against a real obfuscated book and a font is not the place to find out",
    "font.subset-refused-restricted": "{count} font(s) declare a restricted licence in their own file (fsType), which forbids embedding them at all, so nothing was cut from them",
    "font.subset-refused-preview-print": "{count} font(s) declare that embedding them is for preview and printing only, so nothing was cut from them",
    "font.subset-refused-no-subsetting": "{count} font(s) declare in their own file that they may not be subset — the one flag that names this operation exactly — so nothing was cut from them",
    "font.subset-refused-unreadable": "{count} font(s) would not say what their licence permits, and a file whose terms cannot be read is a file this program does not modify",
    "font.subset-refused-failed": "{count} font(s) could not be cut down safely, so they were kept whole",
    "font.subset-unavailable": "{count} font(s) were not cut down at all, because this build has no font library (fontTools) to do it with — nothing about the fonts was tried or decided; install the declared dependencies and run again",
    "font.subset-refused-no-gain": "{count} font(s) came out no smaller than they went in, so the originals were kept",
    ...
    "package.not-a-book": "this file is not a book: it is a {kind} document, which another part of this program reads. The rebuild repairs books and was given nothing to repair, so nothing was written",
...
CATALOGUE_PL: dict[str, str] = {
    ...
    'typography.ellipsis-kept': '{count} {count:ciąg trzech kropek został|ciągi trzech kropek zostały|ciągów trzech kropek zostało} dokładnie tak, jak w pliku — taka była odpowiedź',
    ...
    'typography.conjunctions-kept': '{count} {count:jednoliterowy spójnik nadal może zostać|jednoliterowe spójniki nadal mogą zostać|jednoliterowych spójników nadal może zostać} na końcu wiersza — taka była odpowiedź',
    ...
    'typography.quotes-kept': '{count} {count:prosty cudzysłów został|proste cudzysłowy zostały|prostych cudzysłowów zostało} dokładnie tak, jak w pliku — taka była odpowiedź',
    ...
    'typography.ranges-kept': '{count} {count:łącznik między liczbami został|łączniki między liczbami zostały|łączników między liczbami zostało} dokładnie tak, jak w pliku — taka była odpowiedź',
    ...
    'css.invalid-value-kept': 'zostawiono {count} {count:deklarację|deklaracje|deklaracji} z niepoprawną wartością „regular”, bo tak brzmiała odpowiedź: ten arkusz ustawia też kursywę albo pogrubienie, a „normal” nadpisałoby to, co te elementy dziedziczą',
    ...
    'css.font-stack-generic-approved': '{count} {count:stos fontów wskazujący|stosy fontów wskazujące|stosów fontów wskazujących} kroje, których rodzaju paczka książki nie deklaruje żadną liczbą, {count:dostał|dostały|dostało} rodzinę zapasową z własnej nazwy fontu albo z powszechnej wiedzy ({generics}) — dopisaną na słowo człowieka',
    ...
    'xhtml.document-language-aligned': 'dokument mówił, że jest napisany w {was}, a publikacja mówi {now}; zgodnie z odpowiedzią dokument też mówi teraz {now} — ma {count} {count:znak|znaki|znaków}, za mało, żeby same były dowodem języka',
    'xhtml.document-language-undecided': 'dokument mówi, że jest napisany w {document}, a publikacja mówi {publication}; ma {count} {count:znak|znaki|znaków}, za mało na dowód w którąkolwiek stronę, a nie było kogo zapytać — zostaje przy swoim',
    ...
    'nav.body-type-carried': 'epub:type="{value}" wydawcy na body dokumentu nawigacji przeniesione słowo w słowo na dokument wygenerowany',
    'package.time-budget-is-a-setting': 'sufit czasu to ustawienie, nie wyrok o książce: podnieś go flagą --time-budget w wierszu poleceń albo polem budżetu czasu w oknie i przebuduj jeszcze raz',
    ...
    'render.checked': '{count} {count:porównanie strony|porównania stron|porównań stron} w {engine} na {documents} z {total} dokumentów kolejności czytania, po {screens} {screens:ekranie|ekrany|ekranów} każdego: tam przebudowana książka rysuje się tak samo jak źródło. Czego nie narysowano, tego nie sprawdzono',
    'render.checked-whole': '{count} {count:porównanie strony|porównania stron|porównań stron} w {engine} na {documents} z {total} dokumentów kolejności czytania, każdy dokument do ostatniego ekranu (najdłuższy: {screens} {screens:ekran|ekrany|ekranów}): wszędzie, gdzie narysowano, przebudowana książka rysuje się tak samo jak źródło',
    ...
    'hyphens.class-left-alone': '{count} słowo/słów z klasy „{confidence}” zostało dokładnie tak, jak w pliku: ta książka nigdzie nie pisze ich bez łącznika, więc dowodu nie ma w żadną stronę, a na pytanie o klasę nikt nie odpowiedział',
    'hyphens.class-kept': '{count} słowo/słów z klasy „{confidence}” zostało dokładnie tak, jak w pliku: ta książka nigdzie nie pisze ich bez łącznika, więc dowodu nie ma w żadną stronę, a odpowiedź padła raz dla całej klasy',
    ...
    'package.attributes-fell': 'wynik niesie mniej niektórych atrybutów, na których polega oprogramowanie wspomagające, niż źródło ({detail}); nic w tej przebudowie nie deklaruje ich usunięcia, więc warto sprawdzić, gdzie się podziały',
    'package.text-uncounted': 'znaków {count} dokumentu/dokumentów nie dało się policzyć po jednej ze stron bilansu ({documents}), więc te dokumenty są wyłączone z obu stron, a bilans znaków bez nich wynosi {before} → {after}. To nie znaczy, że straciły tekst — znaczy, że ta liczba ich nie obejmuje',
    'paragraphs.empty-runs-kept': '{count} {count:pusty akapit|puste akapity|pustych akapitów} w {runs} {runs:ciągu|ciągach|ciągach} — dwa i więcej pod rząd, na brzegu dokumentu albo przy nagłówku — zostaje tak, jak ma plik, jak wybrano; usunięcie zabrałoby z nich {removable}, bo ciąg do {height} między akapitami zachowuje wysokość, a {breaks} {breaks:pojedyncza pusta linia|pojedyncze puste linie|pojedynczych pustych linii} między akapitami to przerwy między scenami, które nigdy nie były w pytaniu',
    'paragraphs.empty-runs-unanswered': '{count} {count:pusty akapit|puste akapity|pustych akapitów} w {runs} {runs:ciągu|ciągach|ciągach} zostaje tak, jak ma plik, bo nikt nie odpowiedział; usunięcie zabrałoby z nich {removable}, ciąg do {height} między akapitami zachowuje wysokość, a {breaks} {breaks:pojedyncza pusta linia|pojedyncze puste linie|pojedynczych pustych linii} między akapitami nigdy nie były w pytaniu',
    'paragraphs.empty-runs-removed': '{count} {count:pusty akapit|puste akapity|pustych akapitów} usuniętych z {documents} {documents:dokumentu|dokumentów|dokumentów}, na słowo człowieka albo stałą odpowiedź partii: ciągów między akapitami skrócono {shortened} (najdłuższy z {longest} do {left}), ciągów na brzegu dokumentu albo przy nagłówku usuniętych w całości {dropped}, zostawionych bez zmiany wysokości {untouched}; tekst jest ten sam co do znaku, {breaks} {breaks:pojedyncza przerwa|pojedyncze przerwy|pojedynczych przerw} między scenami nietknięte, a wpis jest w bilansie zmian',
    'paragraphs.empty-runs-left': '{count} {count:pusty akapit|puste akapity|pustych akapitów} w {document} jednak nie usunięto: to, co zostało, nie było tym samym tekstem albo nie wyszły dokładnie zaplanowane akapity, więc dokument wrócił do stanu sprzed zmiany',
    ...
    'render.pdf-compared': '{count} {count:stronę książki stałej porównano|strony książki stałej porównano|stron książki stałej porównano} z tymi samymi stronami PDF-a — źródło narysowane przez PDFium, książka przez {engine}, tusz liczony blokami; brakowało najwyżej {worst} tuszu strony, wobec progu {limit} zmierzonego na stronach wiernych i uszkodzonych',
    'render.pdf-page-differs': 'na tej stronie brakuje więcej tuszu strony źródłowej, niż pozwala zmierzony próg — {detail}',
    'package.pdf-characters-lost': 'znaków narysowanych w PDF-ie nie ma w przebudowanej książce: {detail}. Policzone drugim czytnikiem, który nic nie wie o wierszach ani kolejności czytania, więc konstrukcja, której konwersja nie obsługuje, nie może się przed nim schować. Nic nie zostało zapisane',
    'package.pdf-characters-changed-on-request': 'drugi czytnik liczy znaki narysowane w PDF-ie, których przebudowana książka nie niesie ({detail}), i każdy z nich jest objęty przebiegiem, o który ktoś poprosił i na który odpowiedział: {rules}. Powiedziane zamiast odmówione, i powiedziane zamiast przemilczane — liczba jest liczbą',
    'package.pdf-characters-lost-beyond-consent': 'znaków narysowanych w PDF-ie nie ma w przebudowanej książce, a przebiegi, o które poproszono, nie tłumaczą ich wszystkich ({rules}): {detail}. Zgoda płaci za tekst, o którym mówi, że go usunęła, znak po znaku; to, co zostaje, jest ubytkiem, o który nikt nie prosił. Nic nie zostało zapisane',
    ...
    'hyphens.kept': '{count} {count:słowo z łącznikiem zostało|słowa z łącznikiem zostały|słów z łącznikiem zostało} dokładnie tak, jak w pliku — taka była odpowiedź',
    ...
    'substitutions.pattern-found': 'ta książka pisze „{wrong}” tam, gdzie ma być „{right}” — w {count} {count:słowie|słowach|słowach}; {evidence} z nich ta sama książka w innych miejscach pisze poprawnie, i to właśnie robi z tego wzorzec, a nie listę literówek. Nic nie zostało zmienione bez pytania',
    'substitutions.replaced': '{count} {count:słowo odzyskało|słowa odzyskały|słów odzyskało} właściwą literę w {documents} {documents:dokumencie|dokumentach|dokumentach} — „{wrong}” stało tam, gdzie ma być „{right}” — potwierdzone raz dla całego wzorca, a nie słowo po słowie',
    'substitutions.left-alone': '„{wrong}” stoi tam, gdzie ma być „{right}”, w {count} {count:słowie|słowach|słowach}, i wszystkie zostały dokładnie tak, jak w pliku — nie było kogo zapytać albo nikt nie odpowiedział',
    'substitutions.kept': '„{wrong}” stoi tam, gdzie ma być „{right}”, w {count} {count:słowie|słowach|słowach}, i wszystkie zostały dokładnie tak, jak w pliku — taka była odpowiedź',
    'substitutions.reverted': '{count} {count:dokument wyszedł|dokumenty wyszły|dokumentów wyszło} z przebiegu podmian ze zmianą większą niż uzgodniona, więc {count:wrócił|wróciły|wróciło} dokładnie w takiej postaci, w jakiej {count:przyszedł|przyszły|przyszło}',
    ...
    'package.unresolved-references': '{count} {count:odnośnik wskazuje|odnośniki wskazują|odnośników wskazuje} kotwicę, której nie ma w dokumencie docelowym ({examples}) — w innym dokumencie książki może być, jak zostawia ją eksport przypisów — a tryb ścisły nie wydaje książki z odwołaniami, których nie potrafi uczciwie rozwiązać; nic nie zapisano',
    ...
    'xhtml.dead-link-removed': 'usunięto {count} {count:element|elementy|elementów} <link> z nagłówka dokumentu {count:wskazujący|wskazujące|wskazujących} na pliki, których w książce nie ma; nic, na co wskazywały, nigdy się nie wczytało',
    ...
    'pictures.undescribed-found': '{count} {count:miejsce z obrazem|miejsca z obrazami|miejsc z obrazami} w {files} {files:pliku|plikach|plikach} bez używalnego tekstu alternatywnego; każdy plik trafił do człowieka z dowodami',
    'pictures.decorative-marked': '{count} {count:miejsce z obrazem oznaczone|miejsca z obrazami oznaczone|miejsc z obrazami oznaczonych} jako ozdobnik (alt="" i role="presentation") — na słowo człowieka',
    'pictures.described': '{count} {count:miejsce z obrazem dostało|miejsca z obrazami dostały|miejsc z obrazami dostało} opis wpisany przez człowieka',
    'pictures.left-alone': '{count} {count:miejsce z obrazem zostaje|miejsca z obrazami zostają|miejsc z obrazami zostaje} bez tekstu alternatywnego w {files} {files:pliku|plikach|plikach}; nikt ich nie opisał (np. {examples})',
    'tables.layout-found': '{count} {count:tabela trzyma|tabele trzymają|tabel trzyma} prozę w jednej komórce albo jednej kolumnie w {docs} {docs:dokumencie|dokumentach|dokumentach} i {count:jest zapowiadana|są zapowiadane|jest zapowiadanych} czytnikowi ekranu jako tabela; lista trafiła do człowieka',
    'tables.layout-marked': '{count} {count:tabela z prozą oznaczona|tabele z prozą oznaczone|tabel z prozą oznaczonych} jako układ strony (role="presentation") — na słowo człowieka; tekst bez zmian',
    'tables.layout-left-alone': '{count} {count:tabela z prozą zostaje|tabele z prozą zostają|tabel z prozą zostaje} zapowiadana jako tabela w {docs} {docs:dokumencie|dokumentach|dokumentach}; nikt ich nie oznaczył',
    'kepub.marked': '{docs} {docs:dokument dostał|dokumenty dostały|dokumentów dostało} znaczniki, których szuka czytnik Kobo: {spans} {spans:span wokół zdania|spany wokół zdań|spanów wokół zdań} i {images} {images:span wokół obrazu|spany wokół obrazów|spanów wokół obrazów} wewnątrz div#book-inner oraz blok stylu, jaki niosą własne pliki Kobo; tekst bez zmian',
    'kepub.already-marked': '{count} {count:dokument już niósł|dokumenty już niosły|dokumentów już niosło} spany zdań Kobo i {count:został|zostały|zostało} jak {count:był|były|były}',
    'kepub.name': 'wynik nazywa się {name}; Kobo wybiera silnik po nazwie i używa własnego tylko dla pliku kończącego się na {extension}, więc ten otworzy jak zwykłego EPUB-a',
    'kepub.first-page-not-a-cover': 'pierwsza strona w kolejności czytania, {name}, to tekst, nie okładka; Kobo wyświetla pierwszą stronę własnego pliku jak okładkę na cały ekran, bez marginesów, a konwerter wzorcowy wstawia przed takim rozdziałem pustą stronę — ten program nie dokłada strony, której wydawca nie napisał',
    ...
    'css.faces-without-src-found': '{count} {count:reguła @font-face nie deklaruje|reguły @font-face nie deklarują|reguł @font-face nie deklaruje} żadnego src, więc nie {count:ma|mają|ma} skąd wziąć fontu i każdy czytnik je pomija',
    'css.faces-without-src-removed': 'usunięto {count} {count:regułę @font-face bez src|reguły @font-face bez src|reguł @font-face bez src}: krój, który nie wskazuje pliku, nie ma czego wczytać',
    'css.faces-without-src-unverified': '{count} {count:reguła @font-face bez src została|reguły @font-face bez src zostały|reguł @font-face bez src zostało} na miejscu: po usunięciu arkusz przestawał być arkuszem',
    ...
    'css.malformed-declaration-converter-kept': '{count} {count:deklaracja zapisana|deklaracje zapisane|deklaracji zapisanych} tak, jak zapisuje się atrybut HTML, {count:nosi|noszą|nosi} podpis konwertera i {count:poszłaby|poszłyby|poszłoby} z resztą jego śmieci; zamiatanie stylów jest wyłączone, więc {count:zostaje|zostają|zostaje} — żaden czytnik nigdy ich nie zastosował',
    'css.publisher-typo-fixed': '{count} {count:deklaracja zepsuta|deklaracje zepsute|deklaracji zepsutych} literówką wydawcy — {names} — {count:naprawiona|naprawione|naprawionych} na słowo osoby; formatowanie, którego żaden czytnik nie stosował, zaczyna obowiązywać',
    'css.publisher-typo-left': '{count} {count:deklaracja ma|deklaracje mają|deklaracji ma} literówkę wydawcy z oczywistą propozycją naprawy; nikt nie odpowiedział, więc {count:została|zostały|zostało} bez zmian — czytniki dalej je ignorują',
    'css.publisher-typo-kept': '{count} {count:deklaracja ma|deklaracje mają|deklaracji ma} literówkę wydawcy z oczywistą propozycją naprawy; odpowiedź brzmiała „zostaw”, więc {count:została|zostały|zostało} bez zmian — czytniki dalej je ignorują',
    ...
    'font.subset': '{count} {count:osadzony font przycięto|osadzone fonty przycięto|osadzonych fontów przycięto} do znaków, których ta książka naprawdę używa — z {before} zrobiło się {after}, oszczędność {saved}',
    'font.subset-tables-dropped': 'przy przycinaniu fontów wypadły tablice, których biblioteka fontów nie umie przenieść przez subset — {names}. Większość to własne notatki edytora fontów albo silniki typograficzne, z których prawie żaden czytnik nie korzysta. Jedna jest warta uwagi: stara tablica kerningu `kern` — tam, gdzie font nie niesie obok niej nowoczesnego kerningu, odstępy między literami mogą się zmienić. Sprawdzenie wyglądu i tak porównuje strony przed i po, i odmawia książki, jeśli coś straciła',
    'font.subset-skipped-obfuscated': 'fonty tej książki przyszły zaciemnione, więc zostały co do bajtu takie, jakie były — przycinanie takich nie było sprawdzone na prawdziwej zaciemnionej książce, a font to nie jest miejsce, żeby się tego dowiadywać',
    'font.subset-refused-restricted': '{count} {count:font deklaruje|fonty deklarują|fontów deklaruje} we własnym pliku licencję zastrzeżoną (fsType), która w ogóle zabrania osadzania, więc nic z nich nie wycięto',
    'font.subset-refused-preview-print': '{count} {count:font deklaruje|fonty deklarują|fontów deklaruje}, że osadzanie służy tylko do podglądu i druku, więc nic z nich nie wycięto',
    'font.subset-refused-no-subsetting': '{count} {count:font deklaruje|fonty deklarują|fontów deklaruje} we własnym pliku zakaz subsetowania — to jedyna flaga, która nazywa tę operację wprost — więc nic z nich nie wycięto',
    'font.subset-refused-unreadable': '{count} {count:font nie powiedział|fonty nie powiedziały|fontów nie powiedziało}, na co pozwala jego licencja, a pliku o nieczytelnych warunkach ten program nie modyfikuje',
    'font.subset-refused-failed': '{count} {count:fontu nie dało się|fontów nie dało się|fontów nie dało się} przyciąć bezpiecznie, więc {count:został|zostały|zostało} w całości',
    'font.subset-unavailable': '{count} {count:font nie został przycięty|fonty nie zostały przycięte|fontów nie zostało przyciętych} wcale, bo ta budowa programu nie ma biblioteki fontów (fontTools), która to robi — o fontach nic nie próbowano ani nie rozstrzygnięto; zainstaluj zadeklarowane zależności i uruchom ponownie',
    'font.subset-refused-no-gain': '{count} {count:font po przycięciu nie był|fonty po przycięciu nie były|fontów po przycięciu nie było} mniejsze niż przed, więc zostały oryginały',
    ...
    'package.not-a-book': 'ten plik nie jest książką: to dokument {kind}, który czyta inna część tego programu. Przebudowa naprawia książki i nie dostała nic do naprawienia, więc nic nie zapisano',
...
DETAILS: dict[str, str] = {
    ...
    "css.font-stack-generic-added":
        "Read from the embedded font file's numbers: its OS/2 table — PANOSE, ten bytes the designer filled in — or, where the designer left that blank, the fixed-pitch flag of its post table. Never inferred from a name, neither the one written in the CSS nor the one inside the font: a name is a word, and a word is put to a person as a question. A stack with no generic family falls back to whatever the reader likes when the embedded font fails to load, which on an e-reader it often does.",
...
DETAILS_PL: dict[str, str] = {
    ...
    "css.font-stack-generic-added":
        "Odczytane z liczb w pliku osadzonej czcionki: z tablicy OS/2 — PANOSE, dziesięć bajtów wpisanych przez projektanta — a gdy projektant zostawił ją pustą, z flagi stałej szerokości w tablicy post. Nigdy nie wywnioskowane z nazwy — ani tej zapisanej w CSS, ani tej wewnątrz fontu: nazwa jest słowem, a słowo idzie do człowieka jako pytanie. Stos bez rodziny generycznej spada na cokolwiek, co czytnik uzna za stosowne, gdy osadzona czcionka się nie wczyta, a na czytniku zdarza się to często.",
    "xhtml.watermark-kept":
