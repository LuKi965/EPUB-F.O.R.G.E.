...
CATALOGUE: dict[str, str] = {
    ...
    "package.memory-refused": "this book is expected to need about {needed} of memory ({text} of it text, which costs twelve times its size once parsed) and the budget here is {budget}; nothing was written, because the alternative is being killed halfway with no report at all",
...
CATALOGUE_PL: dict[str, str] = {
    ...
    'package.memory-refused': 'ta książka będzie potrzebowała około {needed} pamięci ({text} z tego to tekst, a tekst po sparsowaniu kosztuje dwanaście razy swój rozmiar), a budżet tutaj to {budget} — nic nie zapisano, bo alternatywą jest zabicie procesu w połowie i brak jakiegokolwiek raportu',
