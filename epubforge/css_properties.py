"""Every CSS property name a conforming parser knows (pillar A of the 0.4 plan).

Vendored from the `known-css-properties` package, version 0.37.0 — the same
dataset stylelint's `property-no-unknown` reads, which is the rule behind the
Calibre-lint gate this pillar answers to. One authority on both sides: the
census must measure the same quantity the program decides on. The list is a
snapshot on purpose (a build must not depend on npm); its provenance and the
gate it serves are written down in the private measurement record
POMIAR-LINT-CALIBRE-2026-08-21-001.

Vendor-prefixed properties (leading `-`) are deliberately NOT listed and NOT
judged: `-epub-hyphens` and `-webkit-column-count` are honoured by real
reading systems, and the lint rule itself skips prefixed names by default.
"""

from __future__ import annotations

#: known-css-properties 0.37.0, 1324 names, sorted.
