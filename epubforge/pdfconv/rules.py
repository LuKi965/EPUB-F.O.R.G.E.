...
CATALOGUE: dict[str, str] = {
    ...
    "pdf.drawing-carried": "{count} vector drawing(s) were carried into the book as a picture of the region they occupy, drawn from the source at {scale}x. This reader cannot draw lines, so it photographs them instead: what the page looks like arrives, the text standing on the drawing stays text beside it ({labelled} place(s)), and the picture has no text alternative until somebody writes one",
    "pdf.drawing-not-carried": "{pages} page(s) carry a drawing made of vector strokes, and there is nothing installed here that can draw one: the drawings are not in the book, and the text printed on them is, gathered where it stood ({labelled} place(s)). This is an optional part — `pip install epub-forge[draw]`, which is `{missing}` — and with it those pages arrive as a picture of what they look like",
    "pdf.text-shown-by-the-picture": "{count} line(s) stand inside a region this book shows a photograph of, so the page does not draw them a second time: the picture is what you see, the text is what you search. They keep their place and their order — selection, find and a screen reader all reach them — and this line is here because a page that quietly stops painting some of its words should say so",
    "pdf.drawing-not-drawn": "{count} of {detected} vector drawing(s) on {pages} page(s) were not drawn — the renderer refused the region, or it was too small to be worth a picture. Those drawings are not in the book; the text printed on them is, gathered where it stood ({labelled} place(s)). The others were carried, and one carried drawing does not stand in for one that was not",
    "pdf.drawing-merged": "{count} vector region(s) were the frame drawn round a picture the page already carries, so they were not carried as a second picture of the same illustration",
    "pdf.links-carried": "{count} link(s) of the source are in the book as links — {internal} to a place inside it, {external} to an address outside — wrapped round the words that carried them on the page",
    "pdf.links-not-carried": "{count} of {total} link(s) of the source are not in the book: {unresolved} lead to a destination the file does not define, {without_text} stand on no text this reader could wrap, {dropped} lead to a page the book has no place for. A link that leads nowhere is worse than plain text, so these are plain text, and this line is where it says so",
    ...
    "pdf.running-heads-unanswered": "{count} running head(s) or page number(s) from the PDF stay in the text, and **nobody said they should**: the setting was `ask` and the question reached no one, so the safe answer was taken. This is not the same as choosing to keep them — it is a question still owed you, and the line is here so that a silence cannot be read back as a decision",
...
CATALOGUE_PL: dict[str, str] = {
    ...
    'pdf.drawing-carried': '{count} rysunków wektorowych przeniesiono do książki jako obraz obszaru, który zajmują, narysowany ze źródła w skali {scale}x. Ten czytnik nie umie kreślić linii, więc je fotografuje: wygląd strony trafia do książki, tekst stojący na rysunku zostaje tekstem obok niego ({labelled} miejsc), a obraz nie ma alternatywy tekstowej, dopóki ktoś jej nie napisze',
    'pdf.drawing-not-carried': 'na {pages} stronach jest rysunek złożony z krzywych, a nie ma tu czym go narysować: rysunków nie ma w książce, jest tekst, który na nich stał, zebrany tam, gdzie stał ({labelled} miejsc). To część opcjonalna — `pip install epub-forge[draw]`, czyli `{missing}` — i z nią te strony trafiają do książki jako obraz tego, jak wyglądają',
    'pdf.text-shown-by-the-picture': '{count} {count:wiersz stoi|wiersze stoją|wierszy stoi} w obszarze, który ta książka pokazuje jako zdjęcie, więc strona nie rysuje ich drugi raz: obraz jest tym, co widać, a tekst tym, czego się szuka. Zachowują swoje miejsce i kolejność — zaznaczanie, wyszukiwanie i czytnik ekranu docierają do nich tak samo — a ten wiersz jest tu dlatego, że strona, która po cichu przestaje malować część swoich słów, powinna to powiedzieć',
    'pdf.drawing-not-drawn': '{count} z {detected} rysunków wektorowych na {pages} stronach nie zostało narysowanych — program rysujący odmówił obszaru albo obszar był za mały, żeby był wart obrazu. Tych rysunków nie ma w książce; jest tekst, który na nich stał, zebrany tam, gdzie stał ({labelled} miejsc). Pozostałe przeniesiono, a jeden przeniesiony rysunek nie zastępuje tego, którego nie ma',
    'pdf.drawing-merged': '{count} obszarów wektorowych było ramką narysowaną wokół obrazu, który strona już niesie, więc nie przeniesiono ich jako drugiego obrazu tej samej ilustracji',
    'pdf.links-carried': '{count} {count:odsyłacz źródła jest|odsyłacze źródła są|odsyłaczy źródła jest} w książce jako {count:odsyłacz|odsyłacze|odsyłacze} — {internal} do miejsca w niej, {external} do adresu poza nią — owinięte wokół słów, które niosły je na stronie',
    'pdf.links-not-carried': '{count} z {total} odsyłaczy źródła nie ma w książce: {unresolved} prowadzi do miejsca, którego plik nie definiuje, {without_text} stoi na tekście, którego ten czytnik nie umiał owinąć, {dropped} prowadzi do strony, dla której książka nie ma miejsca. Odsyłacz prowadzący donikąd jest gorszy niż zwykły tekst, więc te są zwykłym tekstem, a ta linia o tym mówi',
    ...
    'pdf.running-heads-unanswered': '{count} {count:wiersz żywej paginy albo numeru strony zostaje|wiersze żywej paginy albo numerów stron zostają|wierszy żywej paginy albo numerów stron zostaje} w tekście, a **nikt tego nie wybrał**: ustawienie brzmiało „pytaj”, pytanie do nikogo nie dotarło, więc przyjęto odpowiedź bezpieczną. To nie to samo co decyzja, żeby je zostawić — to pytanie nadal czeka na odpowiedź, a ten wiersz jest po to, żeby milczenia nie dało się odczytać jako wyboru',
