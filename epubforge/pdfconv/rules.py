...
CATALOGUE: dict[str, str] = {
    ...
    "pdf.drawing-not-drawn": "{pages} page(s) carry a drawing made of vector strokes and the renderer would not draw them — a region too small to be worth a picture, or a page it refused. The drawings are not in the book; the text printed on them is, gathered where it stood ({labelled} place(s))",
...
CATALOGUE_PL: dict[str, str] = {
    ...
    'pdf.drawing-not-drawn': 'na {pages} stronach jest rysunek złożony z krzywych, a program rysujący ich nie narysował — obszar za mały, żeby był wart obrazu, albo strona, której odmówił. Rysunków nie ma w książce; jest tekst, który na nich stał, zebrany tam, gdzie stał ({labelled} miejsc)',
