...
def forbidden_characters(root) -> int:
    ...
    for element in iter_elements(root):
        for value in (element.text, element.tail):
            if value:
                found += len(xmlchars.FORBIDDEN.findall(value))
    return found
...
def _drop_forbidden(root) -> None:
    ...
    for element in iter_elements(root):
        if element.text and xmlchars.FORBIDDEN.search(element.text):
            element.text = xmlchars.legal(element.text)
        if element.tail and xmlchars.FORBIDDEN.search(element.tail):
