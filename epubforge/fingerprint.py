...
SIGNATURES: dict[str, tuple[Signal, ...]] = {
    "calibre": (
        Signal(r"calibre:series", 0.9, "package", "calibre:series"),
        Signal(r'class="[^"]*\bcalibre\d+\b', 0.85, "markup", "class=calibreN"),
        Signal(r"\.calibre\d+\b", 0.85, "css", ".calibreN"),
        ...
        Signal(r"(?i)\bcalibre\b(?!:)", 0.8, "package", "calibre@package"),
        # The bare word anywhere else is weak on purpose: it is a real word and
        # it turns up in books about typesetting.
        Signal(r"(?i)\bcalibre\b(?!:)", 0.3, "any", "calibre"),
    ),
    "indesign": (
        Signal(r"_idGenParaOverride", 0.95, "any", "_idGenParaOverride"),
        Signal(r"_idGenObjectStyle", 0.95, "any", "_idGenObjectStyle"),
        Signal(r"(?i)InDesign", 0.8, "package", "InDesign@package"),
        Signal(r"(?i)InDesign", 0.35, "any", "InDesign"),
    ),
    "word": (
        Signal(r"\bMsoNormal\b", 0.9, "any", "MsoNormal"),
        Signal(r"\bmso-[a-z-]+\s*:", 0.85, "any", "mso-*"),
        Signal(r"<o:p>", 0.8, "markup", "<o:p>"),
        ...
        Signal(r"\blst-kix_", 0.9, "any", "lst-kix_"),
        Signal(r"\bdocs-internal-guid", 0.9, "any", "docs-internal-guid"),
        Signal(r"themes\.googleusercontent\.com", 0.85, "any", "themes.googleusercontent.com"),
    ),
    "sigil": (
        Signal(r"(?i)Sigil version", 0.95, "any", "Sigil version"),
        Signal(r'class="[^"]*\bsgc-', 0.8, "markup", "class=sgc-*"),
    ),
    "vellum": (
        Signal(r"(?i)\bvellum\b", 0.9, "package", "Vellum@package"),
        ...
        Signal(r"(?i)\bvellum\b", 0.25, "any", "vellum"),
    ),
    "pressbooks": (
        Signal(r"(?i)pressbooks", 0.9, "any", "pressbooks"),
        Signal(r'class="[^"]*\bwp-', 0.4, "markup", "class=wp-*"),
    ),
    "pdf-or-ocr": (
        Signal(r"(?i)ABBYY", 0.9, "any", "ABBYY"),
        Signal(r"(?i)pdftohtml", 0.9, "any", "pdftohtml"),
        Signal(r"(?i)PDF Reflow conversion", 0.9, "any", "PDF Reflow conversion"),
        ...
        Signal(r"\bindex-\d+_\d+\.(?:png|jpe?g)\b", 0.7, "any", "index-P_N.png"),
    ),
    "from-mobi": (
        Signal(r"\bfilepos\d+", 0.9, "any", "filepos"),
        Signal(r"kindle:pos", 0.9, "any", "kindle:pos"),
    ),
    "gutenberg": (
        Signal(r"x-ebookmaker", 0.95, "any", "x-ebookmaker"),
        Signal(r"(?i)Project Gutenberg", 0.85, "any", "Project Gutenberg"),
    ),
    "self-publishing": (
        Signal(r"(?i)\b(epubli|lulu\.com|blurb|draft2digital)\b", 0.8, "any",
               "epubli|lulu|blurb|draft2digital"),
    ),
}
...
def identify(*, package: str = "", markup: str = "", css: str = "") -> list[Trace]:
    ...
    for name in SIGNATURES:
        ...
        for pattern, signal in _patterns(name):
            if not pattern.search(haystacks.get(signal.place, "")):
                continue
